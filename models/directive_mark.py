...
class DirectiveMark:
    ...
    get_directive_rows); not in the note list by default - Ctrl+N on a
    Region 1 directive row adds it there as a score-level point row at its
    own position (MusicData.toggle_directive_in_note_list), per-score in the
    .rsc. Populated by TimelineBuilder as a side effect of build(); MIDI/GP/UG
    builders stub this empty, like every other MusicXML-only marker list."""
