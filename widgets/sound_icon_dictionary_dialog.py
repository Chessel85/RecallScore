...
10.1): every sound Recall Score can make, with a Play button to hear it on
demand. Always enabled, with or without a score loaded (decided in review -
this is a reference, not something tied to the current file).
...
class SoundIconDictionaryDialog(QDialog):
    ...
    def __init__(self, parent=None, entries: Optional[List[SoundCatalogEntry]] = None):
        ...
        for entry in self._entries:
            self.sound_list.addItem(f"{entry.name}: {entry.meaning}. Heard when: {entry.when}.")
